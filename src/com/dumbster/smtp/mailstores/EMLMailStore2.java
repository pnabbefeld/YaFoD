 ...
 * The messages are stored in order but getMessages won't return messages in the
 * same order they were received.
 */
public class EMLMailStore2 implements MailStore {
    ...
    public void addMessage(MailMessage message) {
        ...
        try {
            ...
            writer.append('\n');
            MailBody body = new MailBody(message.getBody(), message.getHeaderValues("Content-Type"));
            writer.append(body.asJoinedString());
            writer.append('\n');

            writer.close();

        } catch (IOException | MessagingException | RuntimeException ex) {
            Logger.getLogger(EMLMailStore2.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
